...
plt.bar(list(str(i) for i in n_tasks), (sigma_1 - min_l) / min_l * 100)
plt.xlabel(r'Processes')
plt.ylabel(r'$\sigma$ / %')
plt.savefig('sigma_procs.png')

plt.figure()
plt.bar(list(str(i) for i in n_tasks), np.mean(times, axis=1), yerr=np.std(times, ddof=1, axis=1))
plt.xlabel('Processes')
plt.ylabel(r'$t$ / s')
plt.savefig('t_procs.png')
